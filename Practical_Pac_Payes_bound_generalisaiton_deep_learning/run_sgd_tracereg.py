...
parser.add_argument('--save_epochs','--list', nargs='*', help='which epochs to save', required=True)
...
parser.add_argument('--trace_factor', type=float, default=0.01, metavar='trace_factor', help='trace regulariser constant')
parser.add_argument('--hess_approx', type=str, default='ggn', metavar='trace matrix approx', help='trace regulariser approximation')
...
if args.hess_approx != 'ggn':
    print('using standard hessian')
    args.hess_approx = 'hess'
# convert types
try:
...
if args.new_schedule:
    thing += "_new"
args.dir += '/'+args.hess_approx+'_tracereg/reg='+str(args.trace_factor)+'_seed='+str(args.seed)+'_lr=' + str(args.lr_init) + thing + '_mom=' + str(args.momentum) + '_wd=' + str(
...
if args.dataset == 'ImageFolder':
    ...
    if args.swag:
        ...
        print('SGD trace regularised training')
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.swag and args.swag_c_epochs < 1 and epoch >= args.swag_start:
        ...
        train_res = utils.train_epoch_tracereg(loaders['train'], model, criterion, optimizer, args.hess_approx, verbose=args.verbose,
                                      swag_model=swag_model, swag_batch_c=int(len(loaders['train']) * args.swag_c_epochs))
    else:
        train_res = utils.train_epoch_tracereg(loaders['train'], model, criterion, optimizer, args.hess_approx, trace_factor,
